...
import { 
  ...
  FileEdit, ClipboardList, Ticket, FlaskConical, BarChart2, 
  Shield, LogOut, Sun, Moon, ChevronRight, ChevronLeft, GraduationCap
} from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { section: 'People', items: [
    ...
    { path: '/admin/faculty', label: 'Faculty', icon: <UserCog size={18} /> },
  ]},
  { section: 'Academics', items: [
    { path: '/admin/curriculum', label: 'Curriculum', icon: <BookOpen size={18} /> },
  ...
  { section: 'Reports', items: [
    { path: '/admin/obe', label: 'OBE Management', icon: <FlaskConical size={18} /> },
    { path: '/admin/reports', label: 'Reports', icon: <BarChart2 size={18} /> },
    { path: '/admin/audit', label: 'Audit Trail', icon: <Shield size={18} /> },
  ]},
];

export default function AdminLayout({ children, collapsed, setCollapsed }) {
    ...
    <div className="admin-layout">
      ...
      <header className={`admin-header${collapsed ? ' collapsed' : ''}`}>
        ...
        <div className="header-right">
          ...
          <div className="admin-avatar" title={admin?.full_name}>
            {admin?.full_name?.charAt(0) || 'A'}
          </div>
        </div>
      </header>
