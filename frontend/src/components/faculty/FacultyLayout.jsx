...
import { 
  LayoutDashboard, FileEdit, FlaskConical, LogOut, 
  Sun, Moon, ChevronRight, ChevronLeft, GraduationCap
} from 'lucide-react';

const NAV_ITEMS = [
  { section: 'Overview', items: [
    { path: '/faculty/dashboard', label: 'Faculty Dashboard', icon: <LayoutDashboard size={18} /> },
  ]},
  { section: 'Grading', items: [
    { path: '/faculty/marks', label: 'Marks Entry', icon: <FileEdit size={18} /> },
  ]},
  { section: 'OBE', items: [
    { path: '/faculty/obe', label: 'CO-PO Mapping', icon: <FlaskConical size={18} /> },
  ]},
];

export default function FacultyLayout({ children, collapsed, setCollapsed }) {
    ...
    <div className="admin-layout">
          ...
          {!collapsed && (
            <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px', borderRadius:'var(--radius-md)', background:'var(--bg-hover)', marginBottom:'8px' }}>
              ...
              <div style={{ flex:1, overflow:'hidden' }}>
                ...
                <div style={{ fontSize:'11px', color:'var(--text-tertiary)' }}>{user?.username || 'Faculty'}</div>
              </div>
            </div>
          )}
          <button
      ...
      <header className={`admin-header${collapsed ? ' collapsed' : ''}`}>
        <div className="header-left">
          ...
          <div>
            <div className="header-title">{currentPage?.label || 'Faculty Panel'}</div>
